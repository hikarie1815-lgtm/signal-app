...
> **v2（ミニマル版）での変更**: 目的を「いつ・どこの現場で・何を借りて・いくらか（レンタル）／
> いつ・どこで・何を処分したか（産廃）の記録」に絞り、以下を撤去した。
> 承認ワークフロー（未確認〜支払済み）・差し戻し／修正依頼・取消申請（→本人と管理者が直接削除、論理削除）・
> 処分完了フラグ・OCR読取候補・写真必須バリデーション（写真は全て任意）・
> 管理者ダッシュボード・操作履歴画面（記録自体はDBに残る）。
> 以下の本文には撤去済み機能の記述が含まれる（初版仕様として保存）。
