...
def seed(uploads_dir: str) -> bool:
    """初回のみ、見積内訳書の41商品を料金マスターへ直接登録する。投入したら True。

    環境対策費／日が原本で空欄の行は None のまま登録し needs_review を付ける
    （料金計算では0円として扱い、管理者が後から単価変更で埋められる）。
    """
    conn = get_db()
    try:
        ...
        for fname, label, rows in PAGES:
            ...
            for name, spec, code, daily, monthly, support, basic, damage in rows:
                ...
                conn.execute(
                    ...
                    "monthly_rate,basic_fee,support_per_day,damage_per_day,needs_review,note,status)"
                    " VALUES(?,?,?,?,?,?,?,?,?,?,?,'approved')",
